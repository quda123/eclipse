...
export async function signInWithUsername(username: string, password: string) {
  ...
  const { data: membership, error: membershipError } = await supabase.from('organization_members').select('role').eq('user_id', data.user.id).eq('organization_id', profile.active_organization_id).maybeSingle()
...
export async function currentRole() {
  ...
  if (!user) return null
  const { data: profile, error: profileError } = await supabase.from('profiles').select('status,must_change_password,active_organization_id').eq('id', user.id).maybeSingle()
  ...
  const { data, error } = await supabase.from('organization_members').select('role').eq('user_id', user.id).eq('organization_id', profile.active_organization_id).maybeSingle()
